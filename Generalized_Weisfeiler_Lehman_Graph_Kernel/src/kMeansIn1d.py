...
class kMeans1d:
    def __init__(self, dataSet: List[float], k: int, numOfIters: int, seed: int = 42):
    ...
    def select_initial_centers(self) -> List[float]:
        distinct_vals = list(set(self.dataset))
        if len(distinct_vals) < self.k:
            raise ValueError(
                f"Not enough distinct values ({len(distinct_vals)}) to form {self.k} clusters."
            )
        return sorted(self.rng.sample(distinct_vals, self.k))

    @staticmethod
    def compute_assignment(val: float, centers: List[float]) -> int:
        return min(range(len(centers)), key=lambda i: abs(val - centers[i]))

    def update_centers(self, assignments: List[int]) -> List[float]:
        ...
        for c in range(self.k):
            ...
            if not points:
                ...
                new_centers.append(sum(points) / len(points))
        return new_centers

    @staticmethod
    def check_for_convergence(old_centers: List[float], new_centers: List[float], tol: float = 1e-9) -> bool:
        return all(abs(oc - nc) <= tol for oc, nc in zip(old_centers, new_centers))

    def get_cluster_dict(self, assignments: List[int]) -> Dict[int, List[float]]:
        cluster_to_vals: Dict[int, List[float]] = {}
